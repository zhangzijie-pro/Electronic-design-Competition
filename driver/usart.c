...
{	
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD,ENABLE);             // GPIO时钟使能
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;                        //PA0为复用推挽输出
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOD, &GPIO_InitStructure);    

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;                        //PD6为浮空输入
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOD, &GPIO_InitStructure);
	
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource5,GPIO_AF_USART2);           //PA0引脚复用为USART2
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource6,GPIO_AF_USART2);           //PA1引脚复用为USART2
 ...
 */
static int 
send_byte_to_usart2(char data)          	                          //TTL通信
{
...
{	
  while(USART_GetFlagStatus(USART2,USART_FLAG_RXNE) == 0);                                                                 
 	usart2.receive_buffer[usart2.counter++]=USART_ReceiveData(USART2); //接收数据
 ...
 */
static int
my_printf2(const char * fmt,...)             		                    //串口2输出
{
	__va_list arg_ptr; 
	char buf[UART_BUFFER_SIZE];
